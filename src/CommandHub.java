 ...
 * @author Petrishin Max and Khramova Olga the best programmers in the world!!!
 * @version 2.01 from 11/03/2020
 */
public class CommandHub {
    ...
    CommandHub() {
        ...
        CommandHelpList.put("prepod_is_coming", "Команда prepod_is_coming вызывает Цопу, который дает работягам +10 к респекту.");
    }
    /**
    ...
    int update_by_id(long id, TreeSet<Organization> collection) {
        ...
        while (itr.hasNext()){
            ...
            if(t.getId() == id){
                ...
                add(temp);
                t.replace(temp.first().getName(), temp.first().getCoordinates(), temp.first().getAnnualTurnover(), temp.first().getEmployeesCount(), OrganizationType.valueOf(temp.first().getType()));
    ...
    void prepod_is_coming() throws InterruptedException {
        System.out.println("Hello, Evgenij Alekseevich! Pucture is coming");
        for (int i = 0; i<80; i++)
        {
            System.out.print(".");
            Thread.sleep(70);
    ...
    void remove_by_id(long id, TreeSet<Organization> collection) {
        ...
        System.out.println("Элемент с заданным id успешно обновлен");
    }
    ...
    void add_if_min(TreeSet<Organization> collection) {
        ...
        if (org.compareTo(collection.first())<0)
        {
            collection.add(org);
